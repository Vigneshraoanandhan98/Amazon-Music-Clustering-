...
# Page Config
st.set_page_config(page_title="Amazon Music Universe", page_icon="C:\\Users\\VICKY\\Desktop\\Guvi\\projects\\Project 4\\download (1).ico", layout="wide")
...
with col_logo:
    st.image("C:\\Users\\VICKY\\Desktop\\Guvi\\projects\\Project 4\\download (1).ico", width=120)
with col_title:
...
fig_vio = px.violin(df.sample(min(5000, len(df))), x="Cluster_Type", y=feat_sel, color="Cluster_Type",
                   box=True, points=False, template="plotly_white",
                   color_discrete_map=color_map)
...
cols_to_show = [c for c in ['name_song', 'name_artists', 'danceability', 'energy', 'loudness', 'speechiness', 'acousticness', 'instrumentalness', 'liveness', 'valence', 'tempo', 'duration_ms'] if c in df.columns]
